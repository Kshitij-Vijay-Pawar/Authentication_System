...
export default function Navbar() {
  ...
  const sendVerificationOtp = async () => {
    try {
      const response = await axios.post(
        ...
        {}, // Empty body since userId is attached by middleware
  ...
  return (
    <nav className="w-full flex justify-between items-center px-4 sm:px-24 absolute top-0 shadow bg-white/30 backdrop-blur-md">
      <Link to="/">
        <img
          src={assets.logo}
          alt="website logo"
          className="w-18 sm:w-20 m-2"
        />
      </Link>
      ...
      {userData ? (
        ...
        >
          Login
          <img src={assets.arrow} alt="arrow right" className="w-5 " />
        </button>
      )}
    </nav>
  );
